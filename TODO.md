---
title: Knowledge Convergence
description: An animated AI knowledge convergence hub component connecting multi-source data feeds (YouTube, Medium, GitHub, LeetCode, PDF/Word documents) into a central target title with glowing bezier beams, energy flow particles, and full light/dark theme support.
---

<ComponentPreview name="knowledge-convergence-demo" />
...
```bash
npx lightswind@latest add knowledge-convergence
```
...
| ---- | ---- | ------- | ----------- | -------- |
| `title` | `string` | `"Gyanaguru 2.0"` | Main target title text on the right. | No |
| `badgeText` | `string` | `"Beta"` | Status badge label beside title. | No |
| `showBadge` | `boolean` | `true` | Toggle badge visibility. | No |
| `sources` | `SourceItem[]` | `-` | Array of customizable input nodes on the left. | No |
| `dotColor` | `string` | `"#10b981"` | Glowing green convergence dot color string. | No |
| `glowIntensity` | `"low" | "medium" | "high"` | `"high"` | Background ambient spotlight glow intensity. | No |
| `theme` | `"light" | "dark" | "system"` | `"system"` | Color theme control. | No |
| `className` | `string` | `-` | Additional CSS classes for root wrapper. | No |
