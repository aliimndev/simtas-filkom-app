...
- [ ] Tombol aksi di tab "Menunggu Review": "Review" → buka modal
- [ ] Filter: Tahun Akademik, Program Studi, Bidang Keahlian
...
- [ ] Form keputusan:
  ...
  ```
- [ ] Dua tombol aksi: "✅ Setujui" dan "❌ Tolak"
- [ ] Setelah approve → langsung buka modal penunjukan pembimbing (chain modal)
...
- [ ] Validasi: minimal 1 dosen dipilih
- [ ] Sukses → toast "Pembimbing berhasil ditetapkan, email notifikasi terkirim"
...
- [ ] Kolom: No, Mahasiswa, Judul, Pembimbing, Status, Tanggal Pengajuan, Progres, Aksi
- [ ] Kolom "Progres": progress bar visual 0–100%
- [ ] Klik baris → halaman detail thesis (lihat semua info, log, dokumen, jadwal)
- [ ] Export button → `GET /reports/theses/export?format=excel` (jika v1.1 sudah tersedia)
...
- [ ] Tombol aksi sesuai status thesis saat ini:
...
- [ ] Tabel per tab dengan info relevan
- [ ] Tombol "Jadwalkan" → modal penjadwalan (sama dengan milik Admin, `ScheduleModal`)
- [ ] Tombol "Ubah Jadwal" → modal reschedule
- [ ] Filter: Tahun Akademik, Program Studi, Tanggal
...
- [ ] Field catatan (opsional): "Pesan selamat / catatan yudisium"
- [ ] Tombol "Tetapkan Lulus" — warna hijau prominent
- [ ] Konfirmasi tegas: "Tindakan ini tidak dapat dibatalkan. Mahasiswa akan dinyatakan LULUS secara resmi."
- [ ] Sukses → toast "Yudisium berhasil. Email selamat terkirim ke mahasiswa."
...
- [ ] Berguna untuk identifikasi mahasiswa yang perlu perhatian khusus

---

## Done Criteria

- [ ] `/kaprodi/thesis-reviews` → tab "Menunggu Review" tampil count badge
- [ ] Modal review → setujui thesis → otomatis buka modal penunjukan pembimbing
- [ ] Modal penunjukan → list dosen diurutkan dari beban terendah
- [ ] Setelah penetapan pembimbing → thesis status berubah ke "Bimbingan"
- [ ] Tolak thesis → field catatan wajib diisi
- [ ] `/kaprodi/seminars` → jadwalkan seminar via modal berfungsi
- [ ] Modal reschedule → perubahan jadwal berhasil, tab pindah ke "Terjadwal"
- [ ] Modal yudisium → checklist prasyarat tampil sebelum tombol aktif
- [ ] Tetapkan yudisium → status berubah ke "Lulus", konfirmasi muncul sebelumnya
- [ ] `/kaprodi/theses` → filter lengkap berfungsi, progress bar akurat
