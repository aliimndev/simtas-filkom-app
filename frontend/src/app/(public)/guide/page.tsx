  ...
  return (
    <div>
      <h2 className="flex items-center gap-2 text-lg font-medium text-[var(--st-text)]">
        <Icon className="h-5 w-5 text-[var(--st-accent-from)]" /> {data.title}
      </h2>
      <div className="mt-6 grid gap-4 sm:grid-cols-2">
        {data.items.map((item, i) => (
          <div key={item.title} className="st-card rounded-2xl p-5">
            <div className="flex items-center gap-3">
              <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md accent-gradient text-xs font-bold text-[var(--st-bg)]">
                {i + 1}
              </span>
              <h3 className="font-medium text-[var(--st-text)]">{item.title}</h3>
            </div>
            <p className="mt-2 text-sm text-[var(--st-muted)]">{item.desc}</p>
          </div>
        ))}
      </div>
    </div>
    ...
    <div>
      ...
      <section className="border-b border-[var(--st-stroke)]">
        <div className="landing-container py-16 text-center">
          ...
          <p className="mx-auto mt-3 max-w-xl text-[var(--st-muted)]">
